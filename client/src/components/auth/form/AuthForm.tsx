...
import { type FieldErrors, type FieldValues, type UseFormRegister, useForm } from 'react-hook-form';
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <Stack spacing={4}>
        {formFields.map((fieldName: string) =>
          renderFormField
            ? renderFormField({ fieldName, formErrors: errors, formRegister: register })
            : null,
        )}
        {children}
      </Stack>
